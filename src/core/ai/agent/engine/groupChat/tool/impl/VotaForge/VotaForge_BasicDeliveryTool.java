 ...
 * VotaForge_标准交付
 */
public class VotaForge_BasicDeliveryTool implements Tool {

    @ToolParameter(description = "面板代码列表，多个用逗号分隔", required = true)
    public static final String PARAM_KEY_PANEL_CODES = "panelCodes";
    ...
    public String getName() {
        return "VotaForge_BasicDeliveryTool";
    }

    @Override
    public String getCnName() {
        return "VotaForge_标准交付";
    }

    @Override
    public String getDescription() {
        return "标准交付（原简装交付）本意是对一个或多个已发布的面板使用AI优化，然后发布到应用。\n" +
                "因此当用户想要对面板使用AI优化的时候，或是调整已存在的面板，都可以使用这个工具。\n" +
                "备注：本工具与画布之间不兼容，是独立于画布的一个工具。\n" +
                "参数描述：\n" +
                "参数1: panelCodes, 文本型，你可以传递一个或多个面板编号，如果是多个则由英文逗号分割拼接。\n" +
                "单独参数实例代表对面板编号分别为123的面板进行标准交付: 123\n" +
                "多个参数示例(代表同时对面板编号分别为123、234、678的三个面板进行标准交付): 123,234,678\n" +
                "用户可能只会提供给你某一个面板的名称（当然，用户大概率会携带错别字、省略字，你需要根据语义判断！）" +
                "因此，调用这个工具前可能需要在此之前调用工具GetWorkCacheTool获得工作缓存内容，从中查看之前交付的面板设计列表，" +
